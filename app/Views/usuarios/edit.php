            ...
            <form action="<?= base_url('usuarios/actualizar/' . $usuario['id']) ?>" method="post">
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" id="alias" name="alias" class="form-control" maxlength="50" value="<?= esc($usuario['alias'] ?? '') ?>" disabled>
                        <small class="text-muted">El alias no se puede modificar una vez creado.</small>
                    </div>
                    <div class="col-md-6">
                    ...
                    <div class="col-md-6">
                        <label for="password" class="form-label">Contrasena (opcional)</label>
                        ...
                        <small class="text-muted">Solo completa este campo si deseas cambiar la contrasena.</small>
                    </div>
                    <div class="col-md-6">
                        <label for="id_rol" class="form-label">Rol</label>
                        <?php $idRol = (string) old('id_rol', (string) ($usuario['id_rol'] ?? '')); ?>
                        <select id="id_rol" name="id_rol" class="form-select" required>
                            <option value="">Selecciona un rol</option>
                            <?php foreach (($roles ?? []) as $rol): ?>
                                <option value="<?= esc($rol['id']) ?>" <?= $idRol === (string) $rol['id'] ? 'selected' : '' ?>>
                                    <?= esc($rol['nombre_rol']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
