...
<div class="container py-4">
    ...
    <div class="card border-0 shadow-sm">
        <div class="card-header bg-warning text-dark">
            <h4 class="mb-0"><?= $heading ?? 'Editar Presupuesto Renglón' ?></h4>
        </div>
        <div class="card-body">
            ...
            <form action="<?= base_url('presupuestos-renglones/actualizar/' . $presupuesto['id']) ?>" method="POST">
                <?= csrf_field() ?>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select <?= session('errors.id_renglon') ? 'is-invalid' : '' ?>"
                        ...
                        <?php foreach ($renglones as $renglon): ?>
                            <option value="<?= esc($renglon['id']) ?>" <?= old('id_renglon', $presupuesto['id_renglon']) == $renglon['id'] ? 'selected' : '' ?>>
                ...
                <div class="mb-3">
                    ...
                    <input type="number"
                           ...
                           id="monto_asignado"
                           name="monto_asignado"
                           value="<?= old('monto_asignado', $presupuesto['monto_asignado']) ?>"
                           step="0.01"
                           min="0"
                           required>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select <?= session('errors.status') ? 'is-invalid' : '' ?>"
                        ...
                        <option value="activo" <?= old('status', $presupuesto['status']) === 'activo' ? 'selected' : '' ?>>Activo</option>
                        <option value="inactivo" <?= old('status', $presupuesto['status']) === 'inactivo' ? 'selected' : '' ?>>Inactivo</option>
                    </select>
                ...
                <div class="mt-4">
                    <button type="submit" class="btn btn-warning">Actualizar Presupuesto Renglón</button>
