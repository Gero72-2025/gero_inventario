...
class PresupuestoRenglonesModel extends Model
{
    ...
    {
        $this->distinct()
            ->select('pr.id, pr.id_presupuesto_division, pr.id_renglon, pr.monto_asignado, pr.saldo_actual, pr.status, pr.created_at, pr.updated_at, COALESCE(r.codigo_renglon, "Sin código") as codigo_renglon, COALESCE(r.descripcion, "Sin descripción") as descripcion, COALESCE(pd.monto_asignado, 0) as presupuesto_monto, COALESCE(pd.saldo_actual, 0) as presupuesto_saldo')
